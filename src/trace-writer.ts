...
export namespace TraceWriter {
  export interface Options extends common.Service.AuthenticationConfig {
    projectId: string;
...
export class TraceWriter extends common.Service {
   ...
   * @constructor
   */
  constructor(logger: common.Logger, config: TraceWriter.Options) {
    super({
      packageJson: pjson,
  ...
  initialize(cb: (err?: Error) => void): void {
    ...
    this.getHostname((hostname) => {
      this.getInstanceId((instanceId) => {
        const labels: TraceWriter.LabelObject = {};
...
export const singleton = {
  create(
    ...
    config: TraceWriter.Options & { forceNewAgent_: boolean },
    cb?: (err?: Error) => void
  ): TraceWriter {
    if (!cb) {
      cb = () => {};
